...
// StatsCard component
const StatsCard = ({ title, value, footer, icon }) => (
  <div className="flex flex-col items-center rounded-md border border-gray-300 bg-white p-4 sm:p-6 w-full sm:w-[48%] lg:w-[30%]">
    {/* Icon */}
    <img src={icon} alt="icon" className="mb-2 h-6 w-6" />
    {/* Title */}
    <h2 className="mb-1 text-lg font-medium">{title}</h2>
    {/* Value */}
    <p className="mb-1 text-2xl font-medium text-gray-800">{value}</p>
    {/* Footer */}
    <p className="px-4 py-1 text-sm text-blue-500 bg-blue-50 rounded">{footer}</p>
  </div>
);

// EarningsStatsChart component
...
const Dashboard = () => {
  ...
  if (error) return <div className="text-center font-medium text-red-500">{error}</div>;

  return (
    <main className="w-full p-4 md:p-6">
      {/* Header */}
      <header className="mb-6 flex flex-col items-start md:flex-row md:items-center justify-between">
        <h1 className="text-2xl font-semibold">Dashboard</h1>
        <div className="mt-4 flex flex-col gap-4 md:mt-0 md:flex-row">
          <button className="bg-teal-600 text-white px-4 py-2 rounded-lg">Manage Webpage</button>
          <Link to="/manage-properties/add-property" className="border border-teal-600 text-teal-600 px-4 py-2 rounded-lg">Add Property</Link>
        </div>
      </header>
      ...
      <section className="flex flex-col lg:flex-row gap-6">
        <section className="flex flex-col w-full lg:w-2/3">
          <div className="rounded-lg bg-white p-6">
            <h2 className="text-xl font-medium mb-4">Properties</h2>
            <div className="flex flex-wrap gap-4">
              <StatsCard title="Total Listings" value={dashboard?.total_listings} footer="2 added today" icon="/dashboard-images/Icon.png" />
              <StatsCard title="Active Listings" value={dashboard?.active_listings} footer="5 Paused listings" icon="/dashboard-images/icon2.png" />
              <StatsCard title="Properties Sold" value={dashboard?.sold_properties} footer="2 sold today" icon="/dashboard-images/Icon3.png" />
            </div>
          </div>

          <div className="mt-6 rounded-lg bg-white p-6">
            <h2 className="text-xl font-medium mb-4">Finances</h2>
            <div className="flex flex-col md:flex-row gap-4">
              <div className="w-full md:w-1/2 border rounded-lg p-4">
                <p className="font-medium text-lg">Available Earnings</p>
                <p className="text-3xl mt-4">₦500,000,000</p>
                <button className="mt-6 border px-4 py-2 rounded-lg text-teal-600 border-teal-600">Withdraw</button>
              </div>
              <div className="w-full md:w-1/2 flex flex-col gap-4">
                <div className="border rounded-lg p-4">
                  ...
                  <p className="text-2xl mt-2">₦100,000,000</p>
                </div>
                <div className="border rounded-lg p-4">
                  <p>Today’s Earnings</p>
                   <span className="text-xs text-[#5FC92E]">
        ...
        {/* Website Stats */}
        <section className="w-full lg:w-1/3 bg-white p-6 rounded-lg">
          <h2 className="text-xl font-medium mb-4">Website Stats</h2>
          <div className="flex items-center justify-between mb-4">
            <p className="text-2xl">1987 views</p>
            </div>
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={dashboard?.newData}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="timestamp" />
              <YAxis />
              <Tooltip />
              <Bar dataKey="profile_views" fill="#389294" />
            </BarChart>
          </ResponsiveContainer>
        </section>
      </section>
