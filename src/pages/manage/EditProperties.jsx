...
const EditProperties = () => {
  ...
  const onSubmit =async (data) => {
    console.log({...data});
    try {
      await Instance2.put(`/properties/${id}`,data)
    ...
    const getProperty = async () => {
      try {
        ...
        reset({
          ...
          addedPhotos:propertyData.common_propertyimages ||'',
          squaremeters: propertyData.squaremeters || '',
          numberOfBedrooms: propertyData.number_of_bedrooms || '',
          numberOfBathrooms: propertyData.number_of_bathrooms || '',
          ...
          installment_payment_price: propertyData.installment_payment_price,
          down_payment: propertyData.down_payment,
          installment_duration: propertyData.installment_duration,
          payment_frequency: propertyData.payment_frequency,
        });
      } catch (error) {
        console.error(error);
      }
  ...
  return (
    <div className="px-2 md:px-8 pt-[25px]">
      <h5> <span className="text-[#677373] tracking-[-0.5px] leading-[24px] text-[18px] font-medium">Manage Properties</span> / <span className="text-[#255A5D] tracking-[-0.5px] leading-[24px] text-[18px] font-medium">Edit Properties</span></h5>
      <h3 className="font-semibold py-5 text-[24px] leading-[32px] tracking-[-1px] text-[#242828]">{`${initialForm ? 'Step 1 of 2' : 'Step 2 of 2'}`}</h3>
      <form onSubmit={handleSubmit(onSubmit)}>{ initialForm ? <div>
      <div className="text-[#242828] bg-white px-4 py-6 rounded-lg">
        ...
        <div >
          ...
          <input  type="text"   className="border border-[#CED3D3] w-full py-[16px] px-[14px] rounded-lg" name='name' placeholder="Enter property name" {...register("name",{required:{value:true,message:'This field is required'},minLength:{value:3,message:"Minimum length is 3"}})}/>
          ...
          <div className="md:flex items-center gap-4 mt-3">
            ...
            <input type="text" name="" id="" value={ selectPropertyType}   onClick={()=> setProperty(!propertyItem)} {...register("propertyType",{required:{value:true,message:'This field is required'}})} placeholder='Select Property Type'  className='basis-1/2 border border-[#CED3D3] w-full py-[16px] px-[14px] rounded-lg' />
            ...
            <input type="text" className="border border-[#CED3D3] w-full py-[16px] px-[14px] rounded-lg" placeholder="Enter property size" {...register("squaremeters",{required:{value:true,message:'This field is required'},pattern:{
              value:/^[0-9]+$/,
          ...
          <div className="basis-1/2">
            ...
            <input type="text" className="border border-[#CED3D3] w-full py-[16px] px-[14px] rounded-lg" placeholder="Number of bedrooms" {...register("numberOfBedrooms",{pattern:{
              value:/^[0-9]+$/,
              message: 'Please enter only numbers',
            }})}/>
            {errors.numberOfBedrooms && <span className='text-red-500'>{errors.numberOfBedrooms.message}</span>}
            </div>
              
            <div className="basis-1/2">
            <label className=" font-medium text-[20px] tracking-[-0.75px] leading-[28px]">Bathroom</label>
            <input type="text" className="border border-[#CED3D3] w-full py-[16px] px-[14px] rounded-lg" placeholder="Number of bathrooms" {...register("numberOfBathrooms",{pattern:{
              value:/^[0-9]+$/,
              message: 'Please enter only numbers',
            }})}/>
          {errors.numberOfBathrooms && <span className='text-red-500'>{errors.numberOfBathrooms.message}</span>}
            </div>
          </div>
          ...
          <input type="text" className="border border-[#CED3D3] w-full py-[16px] px-[14px] rounded-lg" placeholder="Enter Keywords" {...register("keywords",{required:{value:true,message:'This field is required'},minLength:{value:3,message:"Minimum length is 3"}})}/>
          {errors.keywords && <span className='text-red-500'>{errors.keywords.message}</span>}
          </div>

          <div className="mt-3">
          <label className=" font-medium text-[20px] tracking-[-0.75px] leading-[28px]">Detailed Description *</label>
          <textarea type="text" className="border border-[#CED3D3] w-full py-[16px] px-[14px] rounded-lg h-[251px] resize-none" placeholder="Write about Property..." {...register("description",{required:{value:true,message:'This field is required'}})}></textarea>
          ...
          <div className="md:flex items-center gap-4 mt-3">
            ...
            <input type="text" name="" id="" value={selectState} placeholder='Select state'  onClick={()=> setSelectLGA(!selectLGA)}  className='basis-1/2 border border-[#CED3D3] w-full py-[16px] px-[14px] rounded-lg' {...register("state",{required:{value:true,message:'This field is required'}})} />
            ...
            <input type="text" className="border border-[#CED3D3] w-full py-[16px] px-[14px] rounded-lg" placeholder="Enter property size" {...register("city",{required:{value:true,message:'This field is required'},minLength:{value:3,message:"Minimum length is 3"}})}/>
          ...
          <input type="text" className="border border-[#CED3D3] w-full py-[16px] px-[14px] rounded-lg" placeholder="Enter full address" {...register("address",{required:{value:true,message:'This field is required'},minLength:{value:3,message:"Minimum length is 3"}})}/>
      ...
      <div className="text-[#242828] bg-white px-2 md:px-4 py-6 rounded-lg mt-6">
        ...
        </div>:<Form2 r={register} set={setValue} control={control} switchPage={handlePageSwitch}  error={errors}  isValid={isValid}/>  } </form>
    </div>
  )
}
